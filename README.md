...
## 5. 대시보드 화면 결과(MainPage.html)
ⅰ) 카테고리별 효자 상품 카드: 누적 판매수가 가장 높은 메뉴를 상단에 황금빛 카드 UI로 배치   
ⅱ) 시간대별 콤보 차트: 매출액(Bar)과 주문수(Line)를 이중 Y축으로 구성하여 피크타임 직관적 분석 가능  
ⅲ) 지점별 월매출 추이: 가중치 데이터가 정상 반영되어 강남점의 독주 및 3~6월의 우상향 성장 곡선 시각화 완료  
